...
pub fn main() -> Result<(), Error> {
    ...
    for line in buffered.lines() {
        ...
        let numbers: Vec<i64> = nums.split_whitespace().map(|x| x.parse().unwrap()).collect(); 

        let eq = Equation{target: target, numbers: numbers};
        equations.push(eq);
    }
...
fn apply_operations(equation: &Equation, current_value: i64, current_index: usize, part: bool) -> bool {
    if current_index == equation.numbers.len() {
        ...
        let b1 = apply_operations(equation, current_value + next_number, current_index + 1, part);
        let b2 = apply_operations(equation, current_value * next_number, current_index + 1, part);
        let b3 = if part {apply_operations(equation, current_value * 10_i64.pow(n) + next_number, current_index + 1, part)} else {false};
